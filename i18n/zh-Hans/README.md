...
> [!NOTE]
> 这是首版编辑性翻译，欢迎母语使用者进行技术审校。发生差异时，以[英文版 README](../../README.md)为准。

<!-- -->

> [!IMPORTANT]
> 本项目所说的 **Graph Engineering（图工程）**特指 AI 智能体系统，而不是图数据库、知识图谱、图 ETL 或图神经网络。
...
**Graph Engineering 目前仍是一个新兴实践术语，并非已经确立的学术领域或公认行业标准。** 本仓库给出的是可公开讨论、检验和修正的工作定义，而不是声称唯一正确的定义。其证据来自多智能体系统、分布式系统、工作流编排和近期语言智能体图研究，而非单一作者或社交媒体帖子。

完整论证请参阅[定义、边界与逐项证据图](../../DEFINITION.md)。

## 最低三项检验

一个系统只有同时满足以下三项承重条件，才属于本仓库的核心范围：

1. **多个边界明确的智能体实例**：至少两个可独立问责的运行时实例，分别拥有上下文、权限、目标或职责，并能在边界内选择下一步行动。
2. **契约化协调**：至少一条显式关系规定控制权、上下文、状态、产物或评估如何传递，以及何时接受、拒绝或升级结果。
3. **承重且可检查的图工件**：拓扑、运行图或图生成策略真实约束执行，并可被版本化、追踪、评估或有意修改。
...
- 📚 [返回英文主目录](../../README.md)：浏览完整的 51 项资源表。

## 翻译与贡献

技术术语应优先保持含义准确，而不是逐字对应。论文、框架和项目标题保持原文；新增资料必须使用规范来源，并符合三项最低检验或清楚说明其基础、底层设施、测量或批判价值。

如果你是母语技术审校者，欢迎提交小型、聚焦的 Pull Request。详见[贡献指南](../../CONTRIBUTING.md)。

## 引用

推荐引用：**He Chaoyue (2026), *Awesome Graph Engineering*.** 作者姓名 **He Chaoyue** 保持不变。
...
机器可读元数据见 [`CITATION.cff`](../../CITATION.cff)。本仓库元数据与原创描述采用 [CC0-1.0](../../LICENSE) 发布；外部链接作品仍遵循各自许可证。
