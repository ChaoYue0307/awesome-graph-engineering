...
| **Topology is an engineering variable, not presentation.** | [DyLAN](https://openreview.net/forum?id=XII0Wp1XA9) constructs task-dependent agent networks; [GPTSwarm](https://proceedings.mlr.press/v235/zhuge24a.html) represents language-agent systems as optimizable graphs; [G-Designer](https://proceedings.mlr.press/v267/zhang25cu.html) and [AFlow](https://openreview.net/forum?id=z5uVAKwmjf) search communication topologies and workflows; a [2026 survey of workflow optimization](https://arxiv.org/abs/2603.22386) reviews this literature as optimization over agentic computation graphs. | Static and dynamic graphs are both valid, but the topology or generating policy must materially influence execution and be open to deliberate evaluation or change. These studies do not prove that learned topology is always better. |
| **Edges require semantics and cost discipline.** | [TarMAC](https://proceedings.mlr.press/v97/das19a.html) learns recipient-aware communication; the [A2A v1.0.0 specification](https://a2a-protocol.org/v1.0.0/specification/) defines discovery, task, message, artifact, update, and version semantics; the [MCP specification 2025-11-25](https://modelcontextprotocol.io/specification/2025-11-25) defines tool and context exchange boundaries. | An edge should specify trigger, allowed payload or artifact, ownership transfer, acceptance evidence, and failure behavior. A2A, MCP, and in-process handoffs solve different edge problems; none is a complete graph architecture by itself. |
...
| **Independent, grounded evidence must control consequential edges.** | [CRITIC](https://openreview.net/forum?id=Sx038qxjek) improves critique using external tools; [Huang et al.](https://openreview.net/forum?id=IkmD3fKBPQ) show that intrinsic self-correction can fail or regress; [VeriMAP](https://aclanthology.org/2026.eacl-long.353/) attaches executable and natural-language passing criteria to dependency-graph subtasks; [Perez's practitioner essay](https://x.com/IntuitMachine/status/2078419526354378975) argues for independent counter-metrics, frozen rules, and reality anchors. | Another agent's opinion is not automatically evidence. Prefer executable tests, external observations, held-out evaluation, provenance, or appropriately independent human judgment. Planner-authored checks can still encode mistakes, and Perez provides contemporary practitioner framing rather than independent validation or sole authority for the rule. |
...
| **More nodes are not presumptively better.** | [MultiAgentBench](https://aclanthology.org/2025.acl-long.421/) measures collaboration processes; [Why Do Multi-Agent LLM Systems Fail?](https://nips.cc/virtual/2025/poster/121528) classifies coordination failures; [Chen et al.](https://proceedings.neurips.cc/paper_files/paper/2024/hash/51173cf34c5faac9796a47dc2fdd3a71-Abstract-Conference.html) show non-monotonic scaling even in simple multi-call compound systems; controlled studies on [agent-system scaling](https://arxiv.org/abs/2512.08296), [relay information loss](https://arxiv.org/abs/2607.16133), and [equal-token single-versus-multi-agent reasoning](https://arxiv.org/abs/2604.02460) test when coordination helps or hurts. | Compare graph designs with the smallest reliable alternative under matched quality, latency, and token budgets. More calls are not equivalent to more agents, but both require causal and cost-controlled evidence. Negative results are necessary evidence against graph-by-default assumptions. |
...
- **Graph data and graph memory engineering are different.** Graph databases, knowledge graphs, graph neural networks, GraphRAG, and graph-shaped episodic or temporal memory organize or compute over connected data. They enter scope only when they directly support the coordination or execution of an agent graph; a single agent with graph memory still fails the plural-agency test. See [COMPARISON.md](COMPARISON.md).
